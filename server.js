// Local development server for the Te Papa Collections browser.
//
...
const routes = {
  ...
  'POST /api/typecounts': require('./api/typecounts'),
...
const server = http.createServer((req, res) => {
  ...
  // --- API routes → the shared serverless handlers ---
  const handler = routes[`${req.method} ${url.pathname}`];
  if (handler) {
    // Match Vercel's request shape: handlers read req.query for the query string.
