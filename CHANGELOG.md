...
- **Home page: a graphic intro panel and a live "collection at a glance" stat band.** The
  ...
  **Total records, Objects, Specimens, Images** and **Downloadable images** — each a live count
  ...
  split into two labelled type-columns — **Humanities** and **Natural history** — sorted by size
  within each. Driven by the `categories` block in `home.json`: `source: "collections"`
  auto-lists them all (with `showCounts`, `sort`, `min`, and `groups: [{ title, collections }]`
  to define the columns), or supply a curated `items` list instead.
...
### Changed
- **New header home button: a network logo + wordmark, replacing the green pill.** The
